...
echo ""
echo -e "${CYAN}=== telegram-mcp install ===${NC}"
echo ""
...
# Step 3: Setup .env
echo "3/5  Cấu hình Telegram credentials..."
ENV_FILE="$(dirname "$SCRIPT_DIR")/.env"

read -p "     Nhập TELEGRAM_BOT_TOKEN (Enter để bỏ qua): " BOT_TOKEN
read -p "     Nhập TELEGRAM_CHAT_ID (Enter để bỏ qua): " CHAT_ID
...
if [ -n "$BOT_TOKEN" ] || [ -n "$CHAT_ID" ]; then
  ...
  echo "     ✅ Credentials đã lưu vào ${ENV_FILE}"
else
...
cfg["mcpServers"]["telegram"] = {"command": "node", "args": [server_path]}
...
echo "  Tools: send_telegram_notification, send_telegram_photo"
echo "  Commands: /telegram-notify, /telegram-photo"
echo ""
